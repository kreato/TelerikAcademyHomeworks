namespace StudentGroupsExtensions
{
    ...
    using System.Linq;
    using StudentGroups;
    class StudentGroupsExtensionsMain
    {
        static void Main()
        {
            ...
            var groupNum2 = students.Where(x => x.GroupNumber == 2).OrderBy(x => x.FirstName).ToList();
            Console.WriteLine("Students from group number 2, grouped by first name:");
            foreach (var student in groupNum2)
            {
